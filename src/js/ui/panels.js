...
export function updateND() {
  ...
  document
    ...
    .forEach((/** @type {HTMLElement} */ b) => b.classList.toggle('active', +b.dataset.depth === state.selectionDepth));

  document.getElementById('ndConnections').innerHTML = same
    .map((id) => {
      const m = nodes[id];
...
export function updatePathBanner() {
  ...
  if (state.pathResult) {
    pathNodes.innerHTML = state.pathResult
      .map((id, i) => {
        const n = nodes[id];
        return (
    ...
    pathNodes.innerHTML =
      `<span class="path-node" style="background:${state.pathStart.color}30;color:${state.pathStart.color}">${state.pathStart.word}</span>` +
