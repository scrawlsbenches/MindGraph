...
export function drawMinimap() {
  ...
  // Viewport rectangle
  const vx0 = -state.camX / state.camZoom,
    vy0 = -state.camY / state.camZoom;
  const vw = state.W / state.camZoom,
    vh = state.H / state.camZoom;
  const rx = ox + (vx0 - minX) * scale,
    ry = oy + (vy0 - minY) * scale;
  const rw = vw * scale,
    rh = vh * scale;
...
mmDiv.addEventListener('touchend', () => {
  mmDragging = false;
});
