...
export function isEdgeHL(e) {
  ...
  if (state.pathResult) {
    for (let i = 0; i < state.pathResult.length - 1; i++) {
      if (
        (e.a === state.pathResult[i] && e.b === state.pathResult[i + 1]) ||
        (e.b === state.pathResult[i] && e.a === state.pathResult[i + 1])
      )
        return true;
    }
    return false;
...
function updatePathParticles() {
  if (!state.pathResult || state.pathResult.length < 2) {
    state.pathParticles = [];
    return;
  }
  if (random() < 0.15) {
  ...
  for (const p of state.pathParticles) {
    ...
    for (const s of segs) {
      if (d <= s.len) {
        ...
        ctx.beginPath();
        ctx.arc(px, py, 5 / state.camZoom, 0, Math.PI * 2);
        ctx.fillStyle = g;
        ctx.fill();
        break;
      }
  ...
  for (const e of edges) {
    ...
    if (isPathEdge) {
      ...
      if (state.selectedNode && state.depthMap && hl) {
        const da = state.depthMap.get(e.a) ?? 99,
          db = state.depthMap.get(e.b) ?? 99;
...
function drawNodes(hasFocus, hasSearch) {
  for (const n of nodes) {
    ...
    if ((n.r > 6 || isPathNode) && hl && n.alpha > 0.5) {
      ...
      ctx.beginPath();
      ctx.arc(n.x, n.y, drawR * (isPathNode ? 4 : 3), 0, Math.PI * 2);
      ctx.fillStyle = g;
      ctx.fill();
    }
    ...
    if (isPathNode) {
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2.5 / state.camZoom;
      ...
      ctx.globalAlpha = 0.35 * n.alpha;
      ctx.stroke();
    }
    if (state.selectedNode && state.depthMap && state.depthMap.has(n.id) && state.depthMap.get(n.id) >= 2) {
      ctx.strokeStyle = n.color;
      ctx.lineWidth = 1 / state.camZoom;
      ctx.globalAlpha = 0.25 * n.alpha;
      ctx.setLineDash([2 / state.camZoom, 2 / state.camZoom]);
      ctx.stroke();
      ctx.setLineDash([]);
    }
    ctx.globalAlpha = 1;
...
function computeLabelVisibility(hasFocus, hasSearch) {
  ...
  for (const lbl of candidates) {
    ...
    for (const p of placed) {
      if (
        lbl.lx + testOff < p.x + p.w + MARGIN &&
        lbl.lx + testOff + testW + MARGIN > p.x &&
        lbl.ly < p.y + p.h + MARGIN &&
        lbl.ly + lbl.h + MARGIN > p.y
      ) {
        overlaps = true;
        break;
      }
...
function drawZoomBadge() {
  if (Math.abs(state.camZoom - 1) > 0.01) {
    ...
    ctx.beginPath();
    ctx.roundRect(12, state.H - 42, 70, 24, 4);
    ctx.fill();
    ctx.font = '11px ' + FONT;
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.textAlign = 'center';
    ctx.fillText(Math.round(state.camZoom * 100) + '%', 47, state.H - 26);
    ctx.textAlign = 'start';
  }
}
